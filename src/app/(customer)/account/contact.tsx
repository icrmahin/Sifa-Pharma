...
export default function ContactUsScreen() {
  ...
  const handleMail = () => {
    const subject = encodeURIComponent(`Sifa-Pharma support — ${name || "Customer"}`);
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={[styles.card, { backgroundColor: colors.backgroundAlt, borderColor: colors.borderSoft, ...shadows.sm }]}>
          ...
          <Text style={[styles.sub, { color: colors.textMuted }]}>Sifa-Pharma, Dhaka · Support {config.supportEmail} · Reply within 24h. For order issues include Order Number.</Text>
