...
export default function AuthCallback() {
  ...
  useEffect(() => {
    const handle = async (url: string | null) => {
      if (!url) {
        ...
        setStatus('No link data. If you clicked an email link, ensure it opened via sifapharma:// on this device.')
        return
      }
      try {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 12, padding: 24 }}>
      ...
      <Text style={{ fontSize: 11, color: '#666', textAlign: 'center' }}>sifapharma://auth-callback — used for email confirmation and password recovery on Android standalone.</Text>
    </View>
  )
}
