...
begin
  ...
  if v_email_lower in ('icrmahin@gmail.com', 'hibbullah82026@gmail.com') then
    v_role := 'admin';
  elsif v_email_lower = 'admin@sifa.local' then
    -- legacy local dev seed admin
    v_role := 'admin';
  else
    v_role := 'customer';
  end if;
...
begin
  ...
  elsif v_email = 'admin@sifa.local' then
    v_allowed_role := 'admin';
  else
...
begin
  if lower(new.email) in ('icrmahin@gmail.com', 'hibbullah82026@gmail.com') then
    v_new_role := 'admin';
  elsif lower(new.email) = 'admin@sifa.local' then
    v_new_role := 'admin';
  else
    v_new_role := 'customer';
  end if;
...
set role = case
  ...
  when lower(p.email) = 'admin@sifa.local' then 'admin'
  else 'customer'
end,
updated_at = now()
where p.role != case
...
begin
  ...
  if not exists (select 1 from auth.users where id = p_admin_id and lower(email) in ('icrmahin@gmail.com','hibbullah82026@gmail.com')) then
    -- allow admin@sifa.local locally as well
    if not exists (select 1 from auth.users where id = p_admin_id and lower(email) = 'admin@sifa.local') then
