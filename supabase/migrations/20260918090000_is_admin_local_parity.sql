-- Local parity: allow admin@sifa.local in is_admin() for deterministic local seed
-- Production allowlist remains icrmahin@gmail.com, hibbullah82026@gmail.com
-- admin@sifa.local is non-routable (.local) and only exists via local seed; safe for dev
-- This makes is_admin() consistent with handle_new_user/enforce/sync which already allow admin@sifa.local
...
as $$
  select exists (
    ...
    where auth.users.id = auth.uid()
      and lower(auth.users.email) in ('icrmahin@gmail.com', 'hibbullah82026@gmail.com', 'admin@sifa.local')
  );
$$;
...
begin
  ...
  if v_email in ('icrmahin@gmail.com', 'hibbullah82026@gmail.com', 'admin@sifa.local') then
    v_role := 'admin';
  else
    v_role := 'customer';
  end if;
...
begin
  ...
  if not exists (select 1 from auth.users where id = p_admin_id and lower(email) in ('icrmahin@gmail.com','hibbullah82026@gmail.com','admin@sifa.local')) then
